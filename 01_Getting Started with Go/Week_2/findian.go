...
func main() {
	...
	preprocessInput := strings.TrimSuffix(strings.ToLower(reader.Text()), "\n")
	if strings.HasPrefix(preprocessInput, "i") &&
		strings.Contains(preprocessInput, "a") &&
		strings.HasSuffix(preprocessInput, "n") {
		fmt.Println("Found!")
	} else {
